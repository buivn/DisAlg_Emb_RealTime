...
void CAN_Init(unsigned char bitrate)
{
	...
	CANIE2  = 0x00;
	CANTCON = 0x00;				// Set prescaler for CAN Timer (= 8)
...
void TTCAN_Scheduler(void *pvParameters)
{
	...
	unsigned char queue; 
	static unsigned char Cycle_Number=0; //counter=0, 
	unsigned char time_mark, base_mark, repeat, type;
	portTickType xLastWakeTime;

	xLastWakeTime = xTaskGetTickCount();	// Time-point of the last wake-up (integer type)
	while(1)
	{
		PORTF |= 0b00100000;
		...
		// Load the time_mark, base_mark, repeat from the trigger matrix
		...
		if (Cycle_Time == time_mark)						//Reached the time mark?
		{
			...
			if (((Cycle_Number-base_mark)%repeat==0 && Cycle_Number>base_mark)||Cycle_Number == base_mark)
			{
				switch (type)
				{
					...
					case 'S':						//send message
					case 's':
						Can_TX(Angle);		// Send the Sensor data in time window 1
						break;
					case 'A':
					case 'a':
					case 'T':
					case 't':	
						queue = pgm_read_byte (&Trigger[Cycle_Time][6]);
						if (queue == PR_DATA) xSemaphoreGive(WakeTaskSe);
						break;
					default: break;
				}
			}
...
void TTCAN_Init()							
{
	CAN_Init(BAUDRATE);
	Can_RX();
	if (xTaskCreate(TTCAN_Scheduler, (signed char *) "TTCAN_Schedule", 256, \
	NULL, tskIDLE_PRIORITY + 3, NULL) == pdPASS) Print("TTCAN Scheduler");
}
...
ISR(CANIT_vect)						
{
	...
	if (CANSTMOB & 0b00100000)					//Received message successfully?
	{
		buffer.length = (CANCDMOB & 0x0F);						//Determine amount/the length of Data
		...
		if (buffer.id == 0x00 && TM != 1)		//receive RM and this node is not Time master
		{	
			TCNT1 = CANTIM - CANTTC;			//configure local clock 
			RXtime = CANTTC;					//Secure/save received time (SOF) of RM
			Cycle_Time = 1;						//Reset cycle time
			//Read Reference-time of Time master
			Ref_time = (buffer.data[0] << 8) + buffer.data[1];
			
			if (sync)		//Receive RM from the second time
			{
			...
			last_ref = Ref_time;	//save the transmitted time (of time master) for the next calculation
			last_rx  = RXtime;
			sync = TRUE;			//Flag for receiving RM more than one time
		}
		CANSTMOB &= 0b11011111;				//Delete Receive-Flag
		CANCDMOB |= 0b10000000;				//Enable again Reception
	} // end of receive frame successful?
