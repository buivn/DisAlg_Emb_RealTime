...
void TTCAN_Scheduler(void *pvParameters)
{
	...
	while(1)
	{
		...
		// read the respective row in Trigger Matrix
		...
		if (Cycle_Time == time_mark)						//Reached the time mark?
		{
			...
			if (((Cycle_Number-base_mark)%repeat == 0 && Cycle_Number>base_mark)||Cycle_Number==base_mark)
			{
				switch (type)
				{
					...
					case 'r':	
						xSemaphoreGive(WakeTaskSe);
						break;
					case 'S':						//send message
					case 's':
						if (xQueueReceive(Choose_Frame_Queue, &Check, 0)) Can_TX(Pusb_uC1);
						else Can_TX(Pusb_uC0);
						break;
					default: break;
				}
			}
		}
		if (Cycle_Time == BASIC_CYCLE-1)	//If the cycle time reaches the last window
		{
			Cycle_Time = 0;
...
void TTCAN_Init()
{
	...
	tskIDLE_PRIORITY + 3, NULL) == pdPASS) Print("TTCAN Scheduler");
}
...
ISR(CANIT_vect)
{
	...
	if (CANSTMOB & 0b00100000)					//Receive message successfully?
	{
		...
		{
			TCNT1 = CANTIM - CANTTC;			// reset local clock (Cycle Time)????
			RXtime = CANTTC;					//Secure/save received time (SOF) of RM
			Cycle_Time = 1;						//Reset Cycle time (with some kind of prescaler)
			...
			if (sync)							//Already received a RM? 
			{
				...
				//Correct Compare value of FeeRTOS-Timers for Drift correction
				OCR1A = (unsigned int) (COMPARE*locdiff/(Ref_time - last_ref));	
			}
			last_ref = Ref_time;	//save Ref_time and RX time to calculate the next basic cycle
			last_rx  = RXtime;
			sync = TRUE;			//Flag for 'Reference-Message is received'
		}
		// Angle message?
		if (buffer.id == 0x001) Angle = buffer;
